...
module.exports = {
    ...
    generateProtofileFromPackageDefinition

}


async function generateProtofileFromPackageDefinition(GrpcPackage) {


    let fullProto = "";


    // Write header of file (proto version and package name)
    let protoHeader = `syntax = "proto3";\npackage ${GrpcPackage.name};\n`;
    ...
    GrpcPackage.services.forEach(function serviceToProtoDefinition(service) {
        ...
        // Complete the service with all the involved rpcs
        let syntaxAllServiceRpcs = '';
        service.grpcs.forEach(grpc => {
            let SyntaxOneRpc = '  rpc ' + grpc.rpcName + ' (' + grpc.argsType.messageName + ') ' + 'returns ' + '(' + grpc.returnType.messageName + ') ' + ' {}\n';


            const messageNames = findValuesForObjectKeyRecursive(grpc, "messageName", 4);
            // if (messageNames)
            //     allMessageTypes.add(...messageNames)

            console.log("MESSAGE NAMES FOUND", messageNames)
        ...
        const syntaxFullService = syntaxServiceBegin + syntaxAllServiceRpcs + syntaxServiceEnd;
        allServicesDesc += syntaxFullService;
    });


    console.log("ALL MESSAGES", allMessageTypes);


    //ESTO FUNCIONA AL PARECER
    function findObjectsWithKey(obj, keyToFind, maxDepth = 4) {
        const results = [];

        function searchObject(currentObj, depth) {
            if (depth > maxDepth)
                return;
            for (const key in currentObj) {
                if (currentObj.hasOwnProperty(key)) {
                    if (key === keyToFind) {
                        ...
                        searchObject(currentObj[key], depth + 1); // Recursively search nested objects
                    }
                }
            }
        }
    ...
    //Busca recursivamente valores para una clave.
    function findValuesForObjectKeyRecursive(obj, keyToFind, maxDepth = 3) {
        const results = [];

        function searchObjectTreeRecursively(currentObj, depth = 1) {
            if (depth > maxDepth)
                return;
            for (const key in currentObj) {
                if (currentObj.hasOwnProperty(key)) {
                    if (key === keyToFind) {
                        console.log("FOUND A MESSAGE TYPE: ", currentObj[key])
                        results.push(currentObj[key]);
                    }
                    else if (typeof currentObj[key] === 'object') {
                        searchObjectTreeRecursively(currentObj[key], depth + 1); // Recursively search nested objects
                    }
                }
            }
        }

        searchObjectTreeRecursively(obj);
        return results;
    }
    ...
    // hay varias opciones:
    // 1) generar todos los mensajes que ha definido un mimso usuario
    // 2) explorar el anidado completo hasta llegar a los tipos de mensaje.
    // 3)


    //messageName -> es lo que indica que hay cada tipo de mensaje


    // messages.forEach(function messageToProtoDefinition(message) {
    //
    //     const syntaxMessageBegin = 'message ' + message.messageName + ' {\n';
    //     // Describe each of the fields within a message
    //     let syntaxFields = '';
    ...
    //             const aFieldDefinitionLine = grpcField.dataType + ' ' + grpcField.dataName + ' = ' + inc + '; \n';
    ...
    fullProto += protoHeader + allServicesDesc;


    console.log(fullProto)
...
async function commitDBToProto(username, savePath, filename) {
...
async function loadProtofileToDescription(protoFilePath, protoPackageToLoad) {
    ...
    try {
        ...
        // Iterate packages in .proto file
        for (let [k, pkgEntry] of Object.entries(protoPackageDefinition)) {
            grpcElements.package = k;

            // Iterate over services and messages in .proto a given package
            for (let [k, entry] of Object.entries(pkgEntry)) {
